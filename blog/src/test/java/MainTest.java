    ...
    public void testTX() throws InterruptedException {
        ...
        for (int i = 0; i < threads.length; i++) {
            ...
            threads[i] = new Thread(() -> {
                try {
                    UserPo newUser = new UserPo(String.valueOf(finalI), "passwd", "roles");
                    userService.testTransaction(newUser);
                } catch (Exception e) {
                    log.error("guyide", e);
                }
                UserPo dbUser1 = userService.queryUserByName(String.valueOf(finalI));
                if (dbUser1 != null) {
                    log.error("wocao:{}", JSON.toJSONString(dbUser1));
                }
            });
        }
        for (int i = 0; i < threads.length; i++) {
